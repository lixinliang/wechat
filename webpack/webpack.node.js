...
let start = () => {
    ...
    if (task == 'dev') {
        step1().then(() => {
            return `webpack-dev-server --inline --quiet --devtool eval --progress --colors --content-base ./src/ --hot --config ./webpack/webpack.dev.js --host 0.0.0.0 --port ${ port } --version=${ version.join('.') }`;
    ...
    if (task == 'build') {
        if (buildjs) {
            ...
            step1().then(() => {
                return `webpack --progress --colors --config ./webpack/webpack.build.js --version=${ version.join('.') }`;
            }).then(step2).then(() => {
                console.log('build complete!'.green);
            }).catch(( err ) => {
                console.log(err.toString().red);
            ...
            fs.readdirSync(packagePath).forEach(( packageVersion ) => {
                ...
                if (fs.existsSync(packageVersionJson) && fs.statSync(packageVersionJson).isFile()) {
                    if (versionRegexp.test(packageVersion)) {
                        entry[`package/${ packageVersion }`] = packageVersionJs;
                    }
                }
            });
            resolve();
