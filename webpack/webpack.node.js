...
let step2 = () => new Promise(( resolve, reject ) => {
    fs.readdir(sourcePath, ( err, files ) => {
        ...
        files.forEach(( filename ) => {
            ...
            if (fs.statSync(file).isDirectory() && filename == 'package') {
                return;
            }
...
let step3 = () => new Promise(( resolve, reject ) => {
    ...
    if (process.argv[3] && regexp.test(process.argv[3])) {
        process.argv[3].split('.').forEach(( value, index ) => {
            version[index] = value;
        });
    }
    console.log('create entry with:', version.join('.').green);
    version.forEach(( value, index ) => {
        if (value === 'x') {
            version[index] = '(0|[1-9](\\d+)?)';
        }
    });
    version = new RegExp('^' + version.join('\\.') + '$');
    // console.log(version);
    // return;
    Promise.all([
            ...
            fs.readdir(entryPath, ( err, files ) => {
                ...
                files.forEach(( filename ) => {
                    ...
                    if (fs.statSync(file).isFile() && path.extname(file) == '.js') {
                        entry[`base/${ path.basename(filename, '.js') }`] = file;
                    }
                });
                resolve();
            });
        ...
        new Promise(( resolve, reject ) => {
            ...
            let packageJson = {};
            let packageJsonPath = path.join(sourcePath, 'package.json');
            fs.readdirSync(packagePath).forEach(( packageVersion ) => {
                let packageJs = path.join(packagePath, packageVersion, 'index.js');
                if (fs.existsSync(packageJs) && fs.statSync(packageJs).isFile()) {
                    let path = `package/${ packageVersion }`;
                    if (version.test(packageVersion)) {
                        entry[path] = packageJs;
                    }
                    packageJson[`${ packageVersion }`] = `${ path }.js`;
                }
            });
            fse.outputJson(packageJsonPath, packageJson, ( err ) => {
                if (err) {
                    reject(err);
                    return;
                }
